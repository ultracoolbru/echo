...
rl.on('line', async (input) => {
  ...
  if (command === ':git') {
    ...
    // Create a fully isolated readline interface for agent mode
    ...
    agentInput.prompt(); // Prompt user for input

    agentInput.on('line', async (line) => {
      if (line.trim().toLowerCase() === 'exit') {
        agentInput.close();
        rl.resume(); // Resume the main CLI after agent mode exit
        rl.prompt();
        return;
      }

      // Process the user input with the LLM and show the result
      const inputPrompt = `
You are Echo Agent helping with code and productivity.
...
${line}
    `;
      ...
      const res = await import('./askLLM').then(mod => mod.askLLM(inputPrompt, false));
      console.log(chalk.green('\nEcho Agent:\n') + res);

      // Log the conversation with the output
      await logConversation(line, res);

      agentInput.prompt(); // Keep prompting for next input in agent mode
    });

    agentInput.on('close', () => {
      rl.resume(); // Resume the main CLI when agent mode is done
      rl.prompt();
    });
  }
  (async () => {
